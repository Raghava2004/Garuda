...
void gps_setup() {
    ...
    gpsSerial.begin(9600); // Default GPS baud rate
    delay(500);
...
void read_gps(void) {
  ...
  if (new_gps_data_available) {                                                                           //If there is a new set of GPS data available.
    if (number_used_sats < 8)digitalWrite(13, !digitalRead(13));                //Change the LED on the STM32 to indicate GPS reception.
    else digitalWrite(13, LOW);                                                              //Turn the LED on the STM solid on (LED function is inverted). Check the STM32 schematic.
    gps_watchdog_timer = millis();                                                                        //Reset the GPS watch dog tmer.
    new_gps_data_available = 0;                                                                           //Reset the new_gps_data_available variable.

    if (ch5_value >= 1600 && waypoint_set == 0) {                                                          //If the flight mode is 3 (GPS hold) and no waypoints are set.
    ...
    if (ch5_value >= 1600 && waypoint_set == 1) {                                                          //If the GPS hold mode and the waypoints are stored.
  ...
  if (gps_watchdog_timer + 1000 < millis()) {                                                             //If the watchdog timer is exceeded the GPS signal is missing.
    if (ch5_value >= 1600) {                                                                  //If flight mode is set to 3 (GPS hold).
      //flight_mode = 2;                                                                                    //Set the flight mode to 2.                                                                                       //Output an error.
    }
  }

  if (ch5_value < 1500 && waypoint_set > 0) {                                                              //If the GPS hold mode is disabled and the waypoints are set.
